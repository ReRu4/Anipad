# scripts/organizer.py — Главный модуль организации торрентов и построения каталога
import os
import sys
import re
import csv
...
def parse_ep_range(ep_str):
    if not ep_str: return 0, 0, 0
    m = re.match(r'(\d+)-(\d+)', ep_str)
    if m:
        start, end = int(m.group(1)), int(m.group(2))
        return start, end, (end - start + 1)
    m1 = re.match(r'(\d+)', ep_str)
    if m1:
        n = int(m1.group(1))
        return 1, n, n
    return 0, 0, 0

def mark_batch_completeness(variants):
    if not variants: return
    local_vars = [v for v in variants if v.get('source') == 'local']
    if not local_vars:
        for v in variants:
            v['is_incomplete'] = False
            v['missing_episodes'] = 0
        return
        
    max_end = max((parse_ep_range(v.get('episodes', ''))[1] for v in local_vars), default=0)
    ...
    for v in variants:
        ...
        start, end, count = parse_ep_range(v.get('episodes', ''))
        if start == 1 and end > 0 and end < max_end:
            v['is_incomplete'] = True
            v['missing_episodes'] = max_end - end
        else:
            v['is_incomplete'] = False
            v['missing_episodes'] = 0
...
def run_organizer():
    print("==================================================")
    print("🌸 ОРГАНИЗАЦИЯ АНИМЕ И ГЕНЕРАЦИЯ КАТАЛОГА 🌸")
    print("==================================================")
    ...
    print(f"1. Закладки: {os.path.basename(csv_path) if csv_path else 'Авто-библиотека'}")
    ...
    def process_one_torrent(src_path):
        t_title, ep, qual, fname = parse_master_torrent(src_path)
        ...
        if os.path.exists(dest_path):
            ...
            new_name = f"{base_no_ext} ({abs(hash(fname)) % 1000}).torrent"
            dest_path = os.path.join(dest_dir, new_name)
        ...
        elif "AnimeVost" in src_path: studio = "AnimeVost"
        elif "StudioBand" in src_path: studio = "Studio Band"
        elif "AniDUB" in src_path: studio = "AniDUB"
    ...
    def get_franchise_info(rus_title, orig_title):
        ...
        if 'фильм' in r_lo or 'movie' in o_lo or 'film' in o_lo: season_label = "Фильм"
        elif 'ova' in r_lo or 'ova' in o_lo: season_label = "OVA"
        elif 'ona' in r_lo or 'ona' in o_lo: season_label = "ONA"
        elif 'спешл' in r_lo or 'special' in o_lo or 'sp' in o_lo: season_label = "Спешл"
        elif m_s:
            s_num = m_s.group(1)
    ...
    # Include missing anime from bookmarks
    ...
    for f_key, data in franchises.items():
        ...
        for s in s_list:
            mark_batch_completeness(s['variants'])
            s['variants'].sort(key=lambda v: (
                1 if v.get('is_incomplete') else 0,
    ...
    print(f"   ИТОГО франшиз: {len(final_franchise_list)}")
    print("✅ Каталог и база успешно построены!")
